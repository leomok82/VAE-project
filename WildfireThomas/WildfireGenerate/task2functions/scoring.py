...
__all__ = ['mse', 'psnr', 'cosine_sim',
           'combined_similarity_score', 'compare_images']
...
def psnr(imageA, imageB):
    ...
    if mse_value == 0:
        # Return a very high PSNR value for identical images
...
def combined_similarity_score(gen_image, obs_image, w1=0.48, w2=0.04, w3=0.48):
    ...
    combined_score = (w1 * mse_score) + \
        (w2 * (1 - cos_sim_score)) + (w3 * (1 / psnr_score))
...
def compare_images(generated_images, observed_images, w1=0.48, w2=0.04, w3=0.48):
    ...
    for obs_image in observed_images:
        scores = [combined_similarity_score(
            gen_image, obs_image, w1, w2, w3) for gen_image in generated_images]
